...
def createShop():
    ...
    return shop.Shop(template, currentPrompt, player, stats, timeService)
...
def test_initialization():
    ...
    # check
    assert shopInstance.template != None
...
def test_run_sell_fish_action():
    # prepare
    shopInstance = createShop()
    shopInstance.template.showOptions = MagicMock(return_value="1")
    shopInstance.sellFish = MagicMock()
    
    # call
    nextLocation = shopInstance.run("What would you like to do?")
...
def test_run_buy_better_bait_action():
    # prepare
    shopInstance = createShop()
    shopInstance.template.showOptions = MagicMock(return_value="2")
...
def test_run_go_to_docks_action():
    # prepare
    shopInstance = createShop()
    shopInstance.template.showOptions = MagicMock(return_value="3")
